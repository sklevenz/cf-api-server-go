...
func TestLogger_Info_TextOutput(t *testing.T) {
	...
	logr.stdWriter = &buf // Redirect output to buffer for testing
...
func TestLogger_Error_TextOutput(t *testing.T) {
	var buf bytes.Buffer

	logr := New(LevelError, false, nil)
	logr.errWriter = &buf // Redirect output to buffer for testing

	logr.Error("Hello World %s", "paul")

	output := buf.String()

	if !strings.Contains(output, "[ERROR] Hello World") {
